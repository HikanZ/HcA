...
if (isset($_POST['signup-submit'])) {
  ...
  if (empty($username) || empty($userlast) || empty($email) || empty($password) || empty($passwordRepeat || empty($admincheck)) ){
    ...
    header("Location: ../menu.users.add.php?error=emptyfields&uid=".$username."&uidlast=".$userlast."&birthdayuid=".$birth."&email=".$email."&functionuid=".$func);
    exit();
  // Se tudo estiver preenchido, validará se o email digitado é um email válido
      ...
      header("Location: ../menu.users.add.php?error=invalidmail&uid=".$username);
      exit();
    ...
    header("Location: ../menu.users.add.php?error=passwordcheck&uid=".$username."&email=".$email);
    exit();
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) { //Se houver algum erro de sql
      header("Location: ../menu.users.add.php?error=sqlerror"); //Retornará à pag anterior
      exit();
    }
      ...
      if ($resultCheck > 0) {
        header("Location: ../menu.users.add.php?error=emailtaken&uid=".$username);
        exit();
      }
      else{//Se depois de todas as verificações e chegou aqui, significa que está tudo ok
        $sql = "INSERT INTO users (uidUsers, uidLastUsers, emailUsers, pwdUsers, roleUsers, birthUsers, adminSystem, cpfUser, createBy, createDate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, DATE(NOW()) )";
        $stmt = mysqli_stmt_init($conn); //Aqui faz a conexão com o banco
        if (!mysqli_stmt_prepare($stmt, $sql)) { //Se houver algum erro de sql
          header("Location: ../menu.users.add.php?error=sqlerror"); //Retornará à pag anterior
          exit();
          ...
          mysqli_stmt_bind_param($stmt, "ssssssiss", $username, $userlast, $email, $hashedPwd, $func, $birth, $adminbin, $cpfUser, $_SESSION['userUid']);
          mysqli_stmt_execute($stmt); // Executa o statement
          header("Location: ../menu.users.add.php?signup=success"); //Retorna à pag anterior com sucesso
          exit();
        }
      }
  ...
  header("Location: ../menu.users.add.php");
  exit();
}
